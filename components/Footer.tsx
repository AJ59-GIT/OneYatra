  ...
  return (
    <footer className="bg-app-card border-t border-app-border pt-16 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12 mb-16">
          <div className="col-span-2 lg:col-span-1">
            <div className="flex items-center gap-2 mb-6">
              <div className="p-2 bg-brand-500 rounded-xl shadow-premium">
            ...
            <p className="text-sm text-app-text opacity-60 mb-6 leading-relaxed">
              India's most trusted unified mobility platform. Connecting billions, one journey at a time.
            </p>
            <div className="flex gap-4">
              {[Facebook, Twitter, Instagram, Youtube].map((Icon, i) => (
                <button key={i} className="p-2 bg-app-card rounded-full border border-app-border text-app-text opacity-50 hover:text-brand-500 hover:border-brand-500 transition-all">
          ...
          {sections.map((section, i) => (
            <div key={i}>
              <h3 className="text-sm font-bold text-app-text uppercase tracking-widest mb-6">{section.title}</h3>
              <ul className="space-y-4">
                {section.links.map((link, j) => (
                  <li key={j}>
                    <button 
                      onClick={() => navigate(link.path)}
                      className="text-sm text-app-text opacity-60 hover:text-brand-500 transition-colors"
                    >
                      {link.label}
                    </button>
                  </li>
        ...
        <div className="pt-8 border-t border-app-border flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex flex-col md:flex-row items-center gap-6 text-xs text-app-text opacity-40">
